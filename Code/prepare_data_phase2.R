...
json_files <- json_files[c(46:length(json_files))]# phase 2
...
df$TotalGrams11[is.na(df$ingredient11_1)][1]<-NA
df$TotalGrams12[is.na(df$ingredient12_1)][1]<-NA
df$TotalGrams14[is.na(df$ingredient14_1)][1]<-NA
df$TotalGrams15[is.na(df$ingredient15_1)][1]<-NA
# df$TotalGrams11[is.na(df$ingredient11_1)]<-NA # 1 does not recognize the recipe, no json
# df$TotalGrams12[is.na(df$ingredient12_1)][c(1,3)]<-NA # 2 does not recognize the recipe, no json
# df$TotalGrams12[is.na(df$ingredient12_1)][2]<-0 # 1 too little on the plate to estimate grams
# df$TotalGrams13[is.na(df$ingredient13_1)]<-NA # 3 does not recognize the recipe, no json
# df$TotalGrams14[is.na(df$ingredient14_1)]<-NA # 3 does not recognize the recipe, no json
# df$TotalGrams15[is.na(df$ingredient15_1)][1]<-NA # 1 does not recognize the recipe, no json
# df$TotalGrams15[is.na(df$ingredient15_1)][2]<-0 # 1 too little on the plate to estimate grams
...
df %>%
  ...
  distinct(ingredient,.keep.all=T) %>% nrow()
...
write.csv(df.converted, "./Output/Data/Final_df_grams_converted_phase2.csv", row.names = F)
...
df.converted %>%
  ...
  # distinct(ingredient,.keep.all=T) %>%
...
dflong<-
  df %>% 
  dplyr::rename(`Recipe Name 1`=TotalGrams11,
                `Recipe Name 2`=TotalGrams12,
                `Recipe Name 3`=TotalGrams13,
                `Recipe Name 4`=TotalGrams14,
                `Recipe Name 5`=TotalGrams15,
                `Ingredient List`=TotalGrams1,
                `Ingredient List 2`=TotalGrams2,
                `Ingredient List 3`=TotalGrams3,
                `Ingredient List 4`=TotalGrams4,
                `Ingredient List 5`=TotalGrams5,
                `Ingredient Quantities`=TotalGrams6, 
                `Ingredient Quantities 2`=TotalGrams7, 
                `Ingredient Quantities 3`=TotalGrams8, 
                `Ingredient Quantities 4`=TotalGrams9, 
                `Ingredient Quantities 5`=TotalGrams10, 
                `Grams Reported`=TotalGramsSelf) %>% #nrow()
  pivot_longer(cols = c(`Recipe Name 1`, `Recipe Name 2`, `Recipe Name 3`, `Recipe Name 4`,`Recipe Name 5`,
                        `Ingredient List`,`Ingredient List 2`,`Ingredient List 3`,`Ingredient List 4`,`Ingredient List 5`,
                        `Ingredient Quantities`,`Ingredient Quantities 2`,`Ingredient Quantities 3`, 
                        `Ingredient Quantities 4`, `Ingredient Quantities 5`), 
               values_to = 'Grams Predicted', names_to = 'Prompt') %>% #tabyl(Prompt)
  ...
  mutate(Classification=factor(Classification,levels=
         ...
         Prompt=factor(Prompt,levels=c('Recipe Name 1','Recipe Name 2','Recipe Name 3','Recipe Name 4','Recipe Name 5',
                                       'Ingredient List','Ingredient List 2','Ingredient List 3','Ingredient List 4','Ingredient List 5',
                                       'Ingredient Quantities','Ingredient Quantities 2','Ingredient Quantities 3',
                                       'Ingredient Quantities 4','Ingredient Quantities 5')))
...
write.csv(dflong_summary, "./Output/Data/Allruns_phase2.csv", row.names = F)
write.csv(dflong, "./Output/Data/Final_grams_phase2.csv", row.names = F)
...
df.converted <- df.converted %>% 
  ...
  mutate(Prompt= case_when(Prompt == 'Prec_1'~'Ingredient List',
                           Prompt == 'Prec_2'~'Ingredient List 2',
                           Prompt == 'Prec_3'~'Ingredient List 3',
                           Prompt == 'Prec_4'~'Ingredient List 4',
                           Prompt == 'Prec_5'~'Ingredient List 5',
                           Prompt == 'Prec_6'~'Ingredient Quantities',
                           Prompt == 'Prec_7'~'Ingredient Quantities 2',
                           Prompt == 'Prec_8'~'Ingredient Quantities 3',
                           Prompt == 'Prec_9'~'Ingredient Quantities 4',
                           Prompt == 'Prec_10'~'Ingredient Quantities 5',
                           Prompt == 'Prec_11'~'Recipe Name 1',
                           Prompt == 'Prec_12'~'Recipe Name 2',
                           Prompt == 'Prec_13'~'Recipe Name 3',
                           Prompt == 'Prec_14'~'Recipe Name 4',
                           Prompt == 'Prec_15'~'Recipe Name 5',
                           
  )) %>% write.csv(., './Output/Data/Final_precision_phase2.csv', row.names = F)
